...
Generating all projection plots requires `N = length(xsol) * npts * 2` evaluations of the objective function, which can belabor the process of automated unit testing.  A different test for mode-finding routines is to recalculate the optimal solution with an "very good" starting point: the current potential solution.  This is the so-called "**refi**ne op**t**izimation" -- or `refit` -- strategy.  
...
```
Thus we can see that the first and second run of `optim` are quite different. However, this does not mean that the refit optimization is now at the mode: 
```{r, fig.width = 10, fig.height = 6, out.width = "97%"}
# projection plots with refined solution
plot(optim_proj(xsol = orefit$xopt, fun = objfun,
                xrng = .5, maximize = FALSE), equalize = FALSE)
```
On the other hand, starting closer to the true solution $\hat \xx = \AA^{-1}\bb$ does result in a better second ift
```{r}
