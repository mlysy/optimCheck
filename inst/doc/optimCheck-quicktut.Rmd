...
```
Thus it is proposed that a combination of `summary` and `diff` methods for projection plots can be useful for constructing automated unit tests.  In this case, plotting itself can be disable by passing `optim_proj` the argument `plot = FALSE`.  See the **`optimCheck/tests`** folder for **`testthat`** examples featuring: 
